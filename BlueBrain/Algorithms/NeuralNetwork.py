...
class NeuralNetwork:
    ...
    def comparing_weights(self, current_result, new_result, output_y, new_weights):
        ...
        for i in range(self.generation_count):
            for t in range(len(output_y)):
                for j in range(len(output_y[0])):
                    total_new[i] = total_new[i] + pow(output_y[t][j] - new_result[i][t][j], 2)
                    total_current[i] = total_current[i] + pow(output_y[t][j] - current_result[i][t][j], 2)

            total_current[i] = total_current[i] * 1 / 2 * len(total_current[i])
            total_new[i] = total_new[i] * 1 / 2 * len(total_new[i])
        for i in range(self.generation_count):
